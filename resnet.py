...
parser.add_argument('--model_path', type=str, default='/home/rnap/data/math180/modelresnet', help='model_path') 
...
def load_model(args, model, optimizer, k):
    model_path = os.path.join(args.model_path, "checkpoint_{}fold.tar".format(k))
...
def train(epoch):
    ...
    for i, (x, y) in enumerate(train_loader):
        ...
        pred_label.extend(predicted_label.tolist()) 
    ...
    true_label = [int(c) for c in true_label]
    pred_label = [int(c) for c in pred_label]
...
def calculate_testing_accuracy():
    ...
    for i in range(n_classes):
        y_true = [1 if label == i else 0 for label in true_label]
        y_scores = [sample[i] for sample in probs_list]

        if sum(y_true) < 2:
            print(f"Skipping class {i} due to insufficient samples.")
            continue
...
for fold, (train_index, test_index) in enumerate(skf.split(images, labels)):
    ...
    model_path = os.path.join(args.model_path, "checkpoint_0fold.tar")
    checkpoint = torch.load(model_path)
    model.load_state_dict(checkpoint['net'])
    optimizer.load_state_dict(checkpoint['optimizer'])
